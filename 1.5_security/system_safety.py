...
def initialise_firestore_or_stop(streamlit_module: Any):
    """Initialise Firestore and stop Streamlit safely if setup fails."""
    try:
        return initialise_firestore(streamlit_module)
    except (ConfigurationError, ExternalServiceError) as exc:
        LOGGER.error("Application startup stopped: %s", exc)
        streamlit_module.error(str(exc))
        streamlit_module.stop()

        # Normally unreachable because st.stop() interrupts execution.
        raise